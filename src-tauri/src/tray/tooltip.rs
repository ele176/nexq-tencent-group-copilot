...
pub fn build_tooltip(
    ...
    match state {
        ...
        TrayState::Recording => {
            ...
            if is_muted {
                format!("NexQ — 会议中（麦克风已静音）· 已进行 {}", elapsed)
            } else {
                format!("NexQ — 会议中 · 已进行 {}", elapsed)
            }
        }
        TrayState::Muted => {
            let elapsed = meeting_start
        ...
        TrayState::Stealth => {
            let elapsed = meeting_start
                .map(|s| format_elapsed(s))
                .unwrap_or_else(|| "00:00".to_string());
            format!("NexQ — 隐身模式 · 已进行 {}", elapsed)
        }
        TrayState::AiProcessing => "NexQ — AI 分析中...".to_string(),
        TrayState::Indexing => {
            if let Some(text) = custom_text {
                format!("NexQ — {}", text)
            } else {
                "NexQ — 正在索引文档...".to_string()
            }
        }
    }
}

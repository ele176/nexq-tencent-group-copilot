...
pub fn build_idle_menu(app: &AppHandle) -> Result<Menu<tauri::Wry>, tauri::Error> {
    ...
    let copy_ai = MenuItem::with_id(app, "copy_ai_answer", "复制上一条 AI 回答", true, None::<&str>)?;
    ...
    let copy_transcript = MenuItem::with_id(app, "copy_transcript", "复制转写记录", true, None::<&str>)?;
...
pub fn build_meeting_menu(app: &AppHandle) -> Result<Menu<tauri::Wry>, tauri::Error> {
    ...
    let sep1 = PredefinedMenuItem::separator(app)?;
    let mute_mic = MenuItem::with_id(app, "toggle_mic", "静音麦克风", true, None::<&str>)?;
    let mute_sys = MenuItem::with_id(app, "toggle_system", "静音系统声音", true, None::<&str>)?;
    let sep2 = PredefinedMenuItem::separator(app)?;

          ...
          {isRecording ? (
            <>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-primary">正在采集音频</span>
              </div>
              <p className="text-meta text-muted-foreground/50">检测到语音后将显示在这里</p>
            </>
          ) : (
            <>
              <Mic className="h-5 w-5 text-muted-foreground/30" />
              <p className="text-xs text-muted-foreground/50">开始会议后即可查看转写记录</p>
            </>
          )}
        </div>
        {isRecording && (
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col">
      ...
      <div className="flex items-center gap-2 mx-1 mb-1 px-2.5 py-1 border-t border-border/10">
        <span className="text-[0.6rem] uppercase tracking-widest text-muted-foreground/40 font-medium">文字</span>
        ...
        <span className="text-[0.6rem] uppercase tracking-widest text-muted-foreground/40 font-medium">翻译</span>
        <div className="flex items-center gap-0.5">
        ...
        <ColorPickerButton value={translationTextColor} onChange={setTranslationTextColor} label="翻译颜色" />
      </div>
  ...
  return (
    <div className={`flex items-center gap-2 rounded-lg px-2.5 py-1.5 transition-colors duration-200 ${
      ...
      <button
        ...
        aria-label={muted ? `取消静音（${label}）` : `静音（${label}）`}
        aria-pressed={muted}
      >

...
export function StatusBar() {
  ...
  const handleEndMeeting = useCallback(async () => {
    try {
      ...
      const msg = err instanceof Error ? err.message : "结束会议失败";
      showToast(msg, "error");
    }
  }, [endMeetingFlow]);
      ...
      <div className="flex flex-wrap items-center justify-between gap-y-1 text-meta text-muted-foreground">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <ServiceIndicator
            ...
            title={`你的语音识别：${youSTT} ${micActive ? "（正在接收音频）" : "（无声音）"}`}
          />
          ...
          <ServiceIndicator
            ...
            title={`对方语音识别：${themSTT} ${systemActive ? "（正在接收音频）" : "（无声音）"}`}
          />
          ...
          {(isStreaming || latencyMs != null) && (
            <>
              ...
              <span className="tabular-nums">
                {isStreaming ? (
                  <span className="animate-pulse text-primary/80">生成中</span>
                ) : (
                  `${latencyMs}ms`
                )}
              </span>
...
function ServiceIndicator({
  ...
  return (
    <div
      ...
      title={title}
      role="status"
      aria-label={`${label}：${active ? "活动中" : "空闲"}`}
    >
      {/* Activity dot */}

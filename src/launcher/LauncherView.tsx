...
// Display labels for AI scenarios (data values stay in English)
...
export function LauncherView() {
  ...
  const handleSetupConfirm = useCallback(async (audioMode: AudioMode, scenario: AIScenario) => {
    ...
    try {
      ...
      showToast("会议已开始", "success");
    } catch (err) {
      const msg = err instanceof Error ? err.message : "会议启动失败";
      setStartError(msg);
      showToast(msg, "error");
    } finally {
      setIsStarting(false);
  ...
  return (
    <div className="flex h-full flex-col bg-background">
        ...
        {activeMeeting && (
          ...
          >
            ...
            <span className="flex items-center gap-0.5 rounded-full bg-success/20 px-1.5 py-0.5 text-meta font-semibold text-success">
              返回 <ArrowRight className="h-2.5 w-2.5" />
            </span>
          </button>
        )}
        ...
        <div className="dash-sidebar flex w-[280px] min-w-[220px] shrink flex-col border-r border-border/10 bg-card/20">
          ...
          <div className="px-3 pt-3 pb-2">
            <div className="group relative">
              ...
              />
              {searchQuery && (
                <button onClick={() => handleSearch("")} className="absolute right-2.5 top-1/2 -translate-y-1/2 rounded-sm p-0.5 text-muted-foreground/40 transition-colors hover:text-foreground cursor-pointer" aria-label="清除搜索">
          ...
          <div className="flex items-center justify-between px-3 pb-2">
            <div className="flex items-center gap-0.5 flex-wrap">
              {([
                ...
                { key: "with_summary", label: "摘要" },
                { key: "online", label: "线上会议" },
                { key: "in_person", label: "线下会议" },
            ...
            {recentMeetings.length > 0 && (
              <button onClick={handleDeleteAll} className="rounded p-1 text-muted-foreground/50 transition-all duration-150 hover:text-destructive hover:bg-destructive/10 active:scale-90 cursor-pointer" aria-label="删除全部会议">
                ...
                <div className="text-left">
                  ...
                  <div className="text-meta font-normal text-white/50">
                    {rememberedMeetingSetup
                      ? `${rememberedMeetingSetup.audioMode === "online" ? "线上会议" : "线下会议"} · ${SCENARIO_LABELS[rememberedMeetingSetup.scenario] ?? rememberedMeetingSetup.scenario.replace("_", " ")}`
                      : "Ctrl+M"
                    }
                  </div>
                </div>
      ...
      {showDeleteAllConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="删除全部会议确认">
          <div className="w-[380px] rounded-2xl border border-border/40 bg-card p-5 shadow-2xl">
            <div className="mb-1 flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-foreground">删除全部会议</h3>
            </div>
            <p className="mb-5 text-xs text-muted-foreground">
              这将永久删除全部 <span className="font-semibold text-foreground">{recentMeetings.length}</span> 个会议及其转写内容。此操作无法撤销。
            </p>
            <div className="flex flex-col gap-2">
              ...
              >
                {isDeletingAll ? (
                  ...
                  `删除全部 ${recentMeetings.length} 个会议`
                )}
              </button>
              <button
      ...
      {/* ═══ CONFLICT MODAL ═══ */}
      {showConflictPrompt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="会议进行中提示">
          <div className="w-[380px] rounded-2xl border border-border/40 bg-card p-5 shadow-2xl">
            <div className="mb-1 flex items-center gap-2">
              <AlertTriangle className="h-4.5 w-4.5 text-warning" aria-hidden="true" />
              <h3 className="text-sm font-semibold text-foreground">会议进行中</h3>
            </div>
            <p className="mb-5 text-xs text-muted-foreground">
              &ldquo;{activeMeeting?.title}&rdquo; 仍在进行中。
            </p>
            <div className="flex flex-col gap-2">

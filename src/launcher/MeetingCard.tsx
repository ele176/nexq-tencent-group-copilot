...
export function MeetingCard({
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      aria-label={`会议：${meeting.title}，${formatRelativeTime(meeting.start_time)}`}
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1.5 flex items-center gap-1.5 flex-wrap">
            ...
            {meeting.audio_mode && (
              ...
              >
                {meeting.audio_mode === "online" ? "线上会议" : "线下会议"}
              </span>
            )}

            {meeting.ai_scenario && (
              <span className="text-[9px] font-medium text-muted-foreground/50 truncate max-w-[80px]">
                {SCENARIO_LABELS[meeting.ai_scenario] ?? meeting.ai_scenario.replace("_", " ")}
              </span>
            )}
        ...
        <div className="flex items-center gap-0.5 opacity-0 transition-opacity duration-150 group-hover:opacity-100">
          {showDeleteConfirm ? (
            <div className="flex items-center gap-1 rounded-lg bg-destructive/10 px-2 py-1">
              <span className="text-meta text-destructive">删除？</span>
              <button

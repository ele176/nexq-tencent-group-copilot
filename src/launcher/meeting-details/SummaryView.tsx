...
export function SummaryView({ meeting, generation, onExport }: SummaryViewProps) {
  ...
  const handleCopy = useCallback(async () => {
    ...
    if (text) {
      ...
      } catch { showToast("复制到剪贴板失败", "error"); }
    }
      ...
      <div className="p-5">
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm text-primary">
            ...
            <span className="font-medium">正在生成摘要...</span>
          </div>
          <button
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16">
      ...
      <p className="mb-1 text-sm font-semibold text-muted-foreground/50">暂无摘要</p>
      <p className="mb-5 text-xs text-muted-foreground/40">根据转写内容生成 AI 摘要</p>
      <button
        onClick={generation.generate}

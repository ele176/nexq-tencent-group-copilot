  ...
  return (
    <div className="border-b border-border/20">
        ...
        <div className="group min-w-0 flex-1">
          {isEditing ? (
            <div className="flex items-center gap-2">
              ...
              />
              <button onClick={handleSaveEdit} disabled={isSaving} className="rounded-md p-1.5 text-success hover:bg-success/10 disabled:opacity-50 cursor-pointer" aria-label="保存标题">
                <Check className="h-4 w-4" aria-hidden="true" />
              </button>
              <button onClick={handleCancelEdit} className="rounded-md p-1.5 text-muted-foreground hover:bg-secondary cursor-pointer" aria-label="取消编辑">
          ...
          <div className="mt-0.5 flex items-center gap-2 text-xs text-muted-foreground/50">
            ...
            <span>&middot;</span>
            <span>{meeting.transcript.length} 个片段</span>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 px-5 pb-2.5 overflow-x-auto">
        <Stat icon={<Clock className="h-3.5 w-3.5" />} label="时长" value={stats.durationDisplay} />
        <Stat icon={<AlignLeft className="h-3.5 w-3.5" />} label="词数" value={stats.wordCount.toLocaleString()} />
        {stats.wordsPerMinute > 0 && (
          <Stat icon={<Zap className="h-3.5 w-3.5" />} label="语速" value={`${stats.wordsPerMinute}/分`} />
        )}
        {stats.speakerBreakdown.map((s) => {
          const Icon = s.speaker === "User" || s.speaker === "Interviewer" ? Mic : Volume2;
          const label = s.speaker === "User" ? "我" : s.speaker;
        ...
        {stats.avgLatencyMs !== null && (
          <Stat icon={<Timer className="h-3.5 w-3.5" />} label="延迟" value={`${stats.avgLatencyMs}ms`} />
        )}
      </div>
    </div>
  );

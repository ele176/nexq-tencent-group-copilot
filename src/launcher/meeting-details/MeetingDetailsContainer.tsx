...
export function MeetingDetails({ meetingId, onBack }: MeetingDetailsProps) {
  ...
  if (loading) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-2">
        ...
        <p className="text-xs text-muted-foreground">正在加载会议数据...</p>
      </div>
    );
  }
  ...
  if (error || !meeting) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-2">
        <p className="text-xs text-red-400">{error || "未找到会议"}</p>
        <button onClick={onBack} className="text-xs text-primary hover:underline cursor-pointer">返回</button>
      </div>
    );
  }

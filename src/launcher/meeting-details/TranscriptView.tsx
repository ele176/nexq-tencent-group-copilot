  ...
  if (segments.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-muted-foreground/50">
        ...
        <p className="text-sm font-medium">暂无转写片段</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative flex flex-1 min-h-0 flex-col">
      ...
      <div className="flex items-center gap-2 rounded-lg bg-muted/20 mx-1 mt-1 mb-1.5 px-2.5 py-1.5">
        ...
        {search.query && search.totalMatches === 0 && (
          <span className="shrink-0 text-xs text-red-400/60">无匹配结果</span>
        )}
        {search.query && (
                    ...
                    if (isSegTranslating) {
                      return (
                        <div className="mt-1 leading-[1.5]" style={{ fontSize: `${translationFontSize}px` }}>
                          <span className="text-muted-foreground/40 animate-pulse">翻译中...</span>
                        </div>
                      );
                    }
                        ...
                        <div className="mt-1 flex items-baseline gap-1.5 leading-[1.5]">
                          ...
                          {isMismatched && onRetranslateSegment && segment.id && (
                            <button
                              ...
                              title={`重新翻译为 ${currentTargetLang?.toUpperCase()}`}
                            >
                    ...
                    if (!segTranslation && onTranslateSegment && segment.id && currentTargetLang) {
                      return (
                        ...
                        >
                          ...
                          翻译为 {currentTargetLang.toUpperCase()}
                        </button>
                      );
                    }
                ...
                {meetingId && (
                  <div className="absolute right-1 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      title={isBookmarked ? "移除书签" : "为此句添加书签"}
                    >
      ...
      <div className="flex items-center gap-3 mx-2 px-3 py-1.5 border-t border-border/10 text-[10px]">
        ...
        <div className="flex items-center gap-1">
          <span className="font-semibold text-muted-foreground/60">正文</span>
          <button
          ...
          >A</button>
          <ColorPickerButton value={transcriptTextColor} onChange={setTranscriptTextColor} label="文字颜色" />
        </div>
        <div className="h-4 w-px bg-border/10" />
        <div className="flex items-center gap-1">
          <span className="font-semibold text-muted-foreground/60">译文</span>
          <button
          ...
          <button
            ...
            className="flex h-5 w-5 items-center justify-center rounded text-[13px] font-bold text-muted-foreground/60 hover:bg-secondary/30 cursor-pointer"
          >A</button>
          <ColorPickerButton value={translationTextColor} onChange={setTranslationTextColor} label="译文颜色" />
        </div>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {onToggleTranslationVisibility && (
              <button
                ...
                title={showTranslations ? "隐藏译文" : "显示译文"}
              >
      ...
      {noteEdit && (
        ...
        >
          ...
          >
            <h3 className="text-sm font-semibold text-foreground mb-3">书签笔记</h3>
            <textarea
              ref={noteInputRef}
              value={noteEdit.note}
            ...
            <div className="mt-3 flex items-center justify-between">
              <span className="text-[10px] text-muted-foreground/40">Ctrl+Enter 保存</span>
...
function TimelineScrubber({
  ...
  const durationLabel = durationSec >= 3600
    ? `${Math.floor(durationSec / 3600)}小时 ${Math.floor((durationSec % 3600) / 60)}分`

...
export function MeetingDetails({ meetingId, onBack }: MeetingDetailsProps) {
  ...
  const loadMeeting = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "加载会议失败");
    } finally {
      setLoading(false);
    }
  }, [meetingId]);
  ...
  if (loading) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm text-muted-foreground">正在加载会议...</p>
      </div>
    );
  }
  ...
  if (error || !meeting) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3">
        <p className="text-sm text-destructive">{error || "未找到会议"}</p>
        <button
          onClick={onBack}
  ...
  if (interactions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-muted-foreground/60">
        ...
        <p className="text-sm font-medium">暂无 AI 交互记录</p>
      </div>
    );
  }
  ...
  if (!summary) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-muted-foreground/60">
        ...
        <p className="text-sm font-medium">暂无摘要</p>

...
export function MeetingSetupModal({ open, onStart, onCancel }: MeetingSetupModalProps) {
  ...
  const handleStart = useCallback(() => {
    ...
    // 群面 Copilot 参数写入 meetingStore，由 startMeetingFlow 消费
    import("../stores/meetingStore").then(({ useMeetingStore }) => {
      useMeetingStore.getState().setPendingGroupCopilot(
        groupEnabled
          ? { enabled: true, caseQuestion: caseQuestion.trim(), durationMinutes }
          : null
      );
    });
    onStart(audioMode, scenario);
  }, [audioMode, scenario, remember, setRememberedMeetingSetup, onStart, groupEnabled, caseQuestion, durationMinutes]);
      ...
      <div className="w-[420px] rounded-2xl border border-border/40 bg-card shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-border/20 px-5 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-semibold text-foreground">开始会议</h2>
        ...
        {!isExpanded && rememberedSetup !== null ? (
          <div className="px-5 py-4 space-y-4">
            ...
            >
              ...
              <span className="text-sm">开始会议</span>
            </button>
          ...
          <div className="px-5 py-4 space-y-5">
            ...
            <div className={`flex items-center justify-between rounded-xl border p-3 transition-all duration-150 ${
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-xs font-semibold text-foreground/80">录制音频</p>
            ...
            {/* ── Group Copilot Toggle ── */}
            <div className={`rounded-xl border transition-all duration-150 ${
              groupEnabled ? "border-primary/30 bg-primary/[0.04]" : "border-border/30 bg-secondary/10"
            }`}>
              <div className="flex items-center justify-between p-3">
                <div className="flex items-center gap-2.5">
                  <Users className={`h-4 w-4 ${groupEnabled ? "text-primary" : "text-muted-foreground/50"}`} />
                  <div>
                    <p className="text-xs font-semibold text-foreground/80">群面 Copilot 战术盘</p>
                    <p className="text-[10px] text-muted-foreground/60 leading-tight mt-0.5">
                      持续分析讨论，自动给出局势/缺口/建议发言
                    </p>
                  </div>
                </div>
                <label className="relative inline-flex cursor-pointer items-center">
                  ...
                  />
                  <div className={`h-5 w-9 rounded-full transition-colors duration-200 after:absolute after:left-[2px] after:top-[2px] after:h-4 after:w-4 after:rounded-full after:bg-white after:shadow-sm after:transition-all after:duration-200 peer-checked:after:translate-x-full ${
                    groupEnabled ? "bg-primary" : "bg-muted-foreground/20"
                  }`} />
                </label>
              </div>

              {groupEnabled && (
                <div className="space-y-2.5 border-t border-border/20 px-3 py-3">
                  <div>
                    <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60">
                      群面题目（可选）
                    </label>
                    <textarea
                      value={caseQuestion}
                      onChange={(e) => setCaseQuestion(e.target.value)}
                      placeholder="粘贴群面题目，AI 会围绕题目分析…"
                      rows={2}
                      className="w-full resize-none rounded-lg border border-border/30 bg-background px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground/40 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary/20"
                    />
                  </div>
                  <div>
                    <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60">
                    ...
                    <div className="flex gap-1.5">
                      {[20, 30, 45, 60].map((m) => (
                        <button
                          ...
                          className={`flex-1 rounded-lg border px-2 py-1.5 text-[11px] font-medium transition-colors cursor-pointer ${
                            durationMinutes === m
                              ? "border-primary/40 bg-primary/10 text-primary"
                              : "border-border/30 bg-secondary/10 text-muted-foreground hover:bg-secondary/20"
                          }`}
                        >
                          {m} 分钟
                        </button>

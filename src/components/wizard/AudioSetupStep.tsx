  ...
  return (
    <div className="flex flex-col items-center">
      <div className="mb-8 text-center">
        ...
        <h2 className="text-2xl font-bold text-foreground">音频设置</h2>
        <p className="mt-2 text-sm text-muted-foreground leading-relaxed">
          为你和对方分别选择音频来源。请记得戴耳机。
        </p>
      </div>
      ...
      <div className="w-full max-w-lg space-y-5">
              ...
              >
                ...
                {r.hasAudio ? (
                  <span className="flex items-center gap-1 text-success">
                    <CheckCircle className="h-3 w-3" /> 有声音
                  </span>
                ) : (
                  <span className="text-muted-foreground/50">静默</span>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="space-y-2.5">
          ...
          >
            <option value="default">
              {loadingDevices ? "正在检测设备..." : "默认输出（回环采集）"}
            </option>
            {devices.outputs.length > 0 && (
              <optgroup label="扬声器 / 输出">
                {devices.outputs.map((d) => (
        ...
        <div className="rounded-xl border border-border/40 bg-secondary/20 p-5">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="w-8 text-xs text-muted-foreground">麦克</span>
              <div

    ...
    <div className="flex flex-col items-center text-center">
      ...
      <div className="w-full max-w-md space-y-3">
        ...
        <DetectionRow
          ...
          detail={
            phaseIndex > 1
              ? `麦克风 ${inputCount} 个，扬声器 ${outputCount} 个`
              : undefined
          }
          ...
          detail={
            phaseIndex > 2
              ? result.ollamaRunning
                ? `正在运行，${result.ollamaModels.length} 个模型`
                : "未检测到"
              : undefined
          }
          ...
          detail={
            phaseIndex > 3
              ? result.lmStudioRunning
                ? `正在运行，${result.lmStudioModels.length} 个模型`
                : "未检测到"
              : undefined
          }
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              {result.ollamaRunning || result.lmStudioRunning ? (
                <span className="text-success">
                  检测到本地模型！AI 功能可以完全离线使用。
                </span>
              ) : (
                <span>
                  未检测到本地模型。可以在下一步配置云端服务商，或稍后安装
                  Ollama。
                </span>
              )}
            </p>

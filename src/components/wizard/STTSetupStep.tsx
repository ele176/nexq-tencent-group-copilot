...
const DEFAULT_LOCAL_MODEL = "streaming-zipformer-multi";
...
const PROVIDERS: ProviderChoice[] = [
  {
    ...
    description: "离线流式中文转写（普通话）— 免费、无需 API Key",
    requiresKey: false,
    free: true,
...
export function STTSetupStep() {
  ...
  useEffect(() => {
    ...
    const withProvider = (party: MeetingAudioConfig["you"], provider: STTProviderType) => ({
      ...party,
      stt_provider: provider,
      local_model_id:
        provider === "sherpa_onnx" ? DEFAULT_LOCAL_MODEL : party.local_model_id,
    });
    const updated: MeetingAudioConfig = {
      ...meetingAudioConfig,
      you: withProvider(meetingAudioConfig.you, youSTT),
      them: withProvider(meetingAudioConfig.them, themSTT),
      preset_name: null,
    };
      ...
      <div className="w-full max-w-lg space-y-6">
        ...
        <div className="rounded-xl border border-success/20 bg-success/5 px-4 py-3">
          <p className="text-xs text-success leading-relaxed">
            <strong>推荐（群面场景）：</strong>
            You 和 Them 都选「Sherpa-ONNX 本地中文」— 离线中文转写，无需任何
            API Key。首次使用前需在 Settings → Local Models 下载中文模型
            （约 296 MB，只需下载一次）。
          </p>
        </div>

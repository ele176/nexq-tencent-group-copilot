  ...
  loadConfig: async () => {
    try {
      ...
      if (!alreadyLoaded && (!resolvedSttProvider || (resolvedSttProvider as string) === "whisper_cpp")) {
        resolvedSttProvider = "sherpa_onnx" as STTProviderType;
        await store.set("sttProvider", resolvedSttProvider);
        console.log("[configStore] Set top-level STT provider to local Sherpa-ONNX");
      }
      ...
      if (!alreadyLoaded && !resolvedMeetingConfig) {
        resolvedMeetingConfig = {
          ...
          preset_name: "腾讯群面 · 本地中文",
        };
        await store.set("meetingAudioConfig", resolvedMeetingConfig);
        console.log("[configStore] Created default meetingAudioConfig (local Chinese Sherpa-ONNX)");
      }

...
interface MeetingState {
  ...
  // 群面 Copilot 待启动参数（由会议设置弹窗写入，startMeetingFlow 消费）
  pendingGroupCopilot: { enabled: boolean; caseQuestion: string; durationMinutes: number } | null;
  ...
  setPendingGroupCopilot: (p: { enabled: boolean; caseQuestion: string; durationMinutes: number } | null) => void;
...
export const useMeetingStore = create<MeetingState>((set, get) => ({
  ...
  setPendingGroupCopilot: (p) => set({ pendingGroupCopilot: p }),

...
const CLOUD_PROVIDERS: ProviderOption[] = [
  {
    ...
    description: "179 种语言 · 每月 200 万字符免费",
    requiresApiKey: true,
    isLocal: false,
    ...
    helpLabel: "获取免费 API 密钥",
  },
  {
    value: "google",
    label: "Google Translate",
    description: "133 种语言 · 每月 $20 免费额度",
    requiresApiKey: true,
    isLocal: false,
  ...
  },
  {
    value: "deepl",
    label: "DeepL",
    description: "31 种语言 · 每月 50 万字符免费",
    requiresApiKey: true,
    isLocal: false,
...
export function TranslationSettings() {
  ...
  function getBadgeState(p: ProviderOption): BadgeState {
    if (p.isLocal) {
      if (p.value === "llm") {
        return { text: "可用", variant: "available" };
      }
      // OPUS-MT — dynamic badge based on model presence
      if (p.value === "opus-mt") {
        if (opusMtHasActive) return { text: "就绪", variant: "ready" };
        if (opusMtDownloadedCount > 0) return { text: `${opusMtDownloadedCount} 个模型`, variant: "available" };
        return { text: "无模型", variant: "no-key" };
      }
      return { text: "未就绪", variant: "no-key" };
    }
    // Cloud providers
    if (testedProviders.has(p.value)) return { text: "就绪", variant: "ready" };
    if (keyStatusMap[p.credentialKey]) return { text: "已有密钥", variant: "available" };
  ...
  const handleSaveAndTest = useCallback(async () => {
    ...
    try {
      ...
      if (result.connected) {
        ...
        setStatusMessage(
          `已连接——支持 ${result.language_count} 种语言（${result.response_ms} 毫秒）`
        );
  ...
  const handleTestLocal = useCallback(async () => {
    ...
    try {
      ...
      setConnectionStatus("error");
      setStatusMessage(e instanceof Error ? e.message : "测试失败");
    }
      ...
      <div className="flex items-center gap-3 rounded-xl border border-primary/30 bg-primary/5 px-5 py-3.5">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-foreground">
            当前：{ALL_PROVIDERS.find((p) => p.value === provider)?.label || provider}
            {" · "}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-border/30 bg-card/50 overflow-hidden">
            ...
            <div className="px-5 pt-4 pb-3 border-b border-border/20 bg-muted/10">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-semibold text-foreground">本地与离线</span>
            ...
            <div className="px-5 pt-4 pb-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-semibold text-foreground">云端服务商</span>
                <span className="ml-auto text-meta text-muted-foreground/60 font-medium uppercase tracking-wider">
                  需要 API 密钥
                </span>
              </div>
            ...
            <div className="rounded-xl border border-border/30 bg-card/50 p-4">
                  ...
                  <input
                    ...
                    placeholder={
                      hasStoredKey && !keyDirty
                        ? "已保存密钥——输入新密钥以替换"
                        : `输入 ${currentProviderOption?.label || selectedProvider} 的 API 密钥`
                    }
                    maxLength={256}
              ...
              {currentProviderOption?.helpUrl && (
                <p className="mt-1.5 text-xs text-muted-foreground">
                  ...
                  >
                    {currentProviderOption.helpLabel || "获取免费 API 密钥"} &rarr;
                  </a>
                </p>
              )}
              ...
              {currentProviderOption?.needsRegion && (
                <div className="mt-3">
                  <label className="mb-1.5 block text-xs font-medium text-foreground">Azure 区域</label>
                  <select
                    value={azureRegion}
          ...
          {canMakeActive && (
            ...
            >
              将 {currentProviderOption?.label || selectedProvider} 设为当前翻译服务商
            </button>
          )}
        </div>
        ...
        <div className="space-y-5">
          {/* ── Language Settings ── */}
          <div className="rounded-xl border border-border/30 bg-card/50 p-4">
            <h3 className="mb-3 text-sm font-semibold text-primary/80">语言</h3>
            ...
            {selectedProvider !== provider && (
              <div className="mb-3 flex items-center gap-2 rounded-lg border border-info/20 bg-info/5 px-3 py-2">
                ...
                <p className="text-xs text-muted-foreground">
                  激活 <span className="font-medium text-foreground">{currentProviderOption?.label}</span> 以查看其支持的语言
                </p>
              </div>
            )}
            ...
            <div className="space-y-4">
              <div>
                <label className="mb-1.5 block text-xs font-medium text-foreground">目标语言</label>
                <select
                  value={targetLang}
              ...
              <div>
                <label className="mb-1.5 block text-xs font-medium text-foreground">源语言</label>
                <select
                  value={sourceLang}
          ...
          {/* ── Behavior Toggles ── */}
          <div className="rounded-xl border border-border/30 bg-card/50 p-4">
            <h3 className="mb-3 text-sm font-semibold text-primary/80">行为</h3>
            <div className="space-y-4">
              {/* Select-to-translate toolbar */}
              <ToggleRow
                label="划词翻译工具条"
                description="在转写文本中选中文字时显示翻译按钮"
              ...
              <ToggleRow
                label="会后回顾中显示翻译"
                description="回顾历史会议时显示翻译工具条和已缓存的翻译"
              ...
              <ToggleRow
                label="缓存翻译结果"
                description="在本地存储已翻译文本，避免重复翻译相同内容"

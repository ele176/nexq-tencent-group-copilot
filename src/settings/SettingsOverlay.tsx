...
const TAB_GROUPS: TabGroup[] = [
  {
    label: "会议",
    items: [
      { id: "meeting_audio", label: "音频与设备", icon: <Headphones className="h-4 w-4" /> },
    ],
  },
  {
    label: "服务商",
    items: [
      { id: "llm", label: "LLM 服务商", icon: <Brain className="h-4 w-4" /> },
      { id: "stt", label: "语音转文字服务商", icon: <Mic className="h-4 w-4" /> },
      { id: "translation", label: "翻译", icon: <Globe className="h-4 w-4" /> },
    ],
  },
  {
    label: "智能",
    items: [
      { id: "ai_actions", label: "AI 操作", icon: <Sparkles className="h-4 w-4" /> },
      { id: "context_strategy", label: "上下文策略", icon: <Database className="h-4 w-4" /> },
      { id: "scenarios", label: "AI 场景", icon: <Theater className="h-4 w-4" /> },
      { id: "noise_presets", label: "降噪预设", icon: <Volume2 className="h-4 w-4" /> },
      { id: "confidence", label: "置信度", icon: <BarChart2 className="h-4 w-4" /> },
    ],
  },
  {
    label: "系统",
    items: [
      { id: "hotkeys", label: "快捷键", icon: <Keyboard className="h-4 w-4" /> },
      { id: "general", label: "通用", icon: <SlidersHorizontal className="h-4 w-4" /> },
...
export function SettingsOverlay({ isModal = false }: SettingsOverlayProps) {
  ...
  const currentTabLabel = TAB_LABELS[activeTab] ?? "设置";
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div className="flex border-b border-border/30 px-2 overflow-x-auto" role="tablist" aria-label="设置导航">
            {ALL_TABS.map((tab) => (
              <button
                key={tab.id}
                role="tab"
  ...
  return (
    <div className="flex h-full w-full bg-background">
      ...
      <aside className="flex w-56 shrink-0 flex-col border-r border-border/30 bg-card/50">
        ...
        {/* Grouped Navigation */}
        <nav className="flex-1 overflow-y-auto px-3 pt-3 pb-1" role="tablist" aria-label="设置导航">

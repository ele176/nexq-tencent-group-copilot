...
const HOTKEY_LABELS: Record<keyof HotkeyConfig, string> = {
  toggle_assist: "切换 AI 助手",
  start_end_meeting: "开始 / 结束会议",
  show_hide: "显示 / 隐藏悬浮窗",
  open_settings: "打开设置",
  escape: "退出 / 取消",
  mode_assist: "模式：助手",
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (conflictingAction) {
        setConflict(
          `「${combo}」已被「${HOTKEY_LABELS[conflictingAction[0]]}」使用`
        );
      ...
      setHotkeys(updated);
      setEditingKey(null);
      showToast(`快捷键已更新：${HOTKEY_LABELS[editingKey]} = ${combo}`, "success");
    };
  ...
  return (
    <div className="space-y-6">
              ...
              (action, idx, arr) => {
                ...
                return (
                  <tr
                    ...
                    tabIndex={0}
                    role="button"
                    aria-label={`修改「${HOTKEY_LABELS[action]}」的快捷键，当前为 ${binding}`}
      ...
      <p className="text-xs text-muted-foreground/70">
        更改会自动保存。全局快捷键需要 Ctrl 或 Cmd 修饰键。
      </p>
    </div>
  );
}

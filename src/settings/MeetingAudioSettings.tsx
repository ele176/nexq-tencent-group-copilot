...
export function MeetingAudioSettings() {
  ...
  const config: MeetingAudioConfig = meetingAudioConfig ?? {
    you: {
      ...
      local_model_id: "streaming-zipformer-multi",
    },
    them: {
      role: "Them",
  ...
  const allDevices: { device: AudioDevice; group: string }[] = [
    ...devices.inputs.map((d) => ({ device: d, group: "麦克风" })),
    ...devices.outputs.map((d) => ({ device: d, group: "扬声器 / 输出" })),
  ];
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <PartyPanel
          label="对方"
          labelSuffix={<span className="text-xs font-semibold uppercase tracking-wide text-purple-400">/&nbsp;现场</span>}
          description="远端参会者（线上）或房间麦克风（线下）"
          role="them"
      ...
      {(["deepgram", "azure_speech"] as STTProviderType[]).some(
        ...
        <div className="flex items-center justify-between rounded-xl border border-border/20 bg-card/40 px-4 py-3">
          <div>
            <p className="text-xs font-medium text-foreground">说话人分离</p>
      ...
      <div className="flex items-center justify-between rounded-xl border border-border/20 bg-card/40 px-4 py-3">
        <div>
          <p className="text-xs font-medium text-foreground">录制到文件</p>
          <p className="mt-0.5 text-meta text-muted-foreground/70">将会议音频保存为 WAV</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="flex flex-col rounded-xl border border-border/20 bg-card/40 p-4">
          <div className="mb-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-foreground">实时监听</span>
              <div className="h-1.5 w-1.5 rounded-full bg-success animate-pulse" title="正在监听所有设备" />
            </div>
            <button
              onClick={loadDevices}
        ...
        <div className="flex flex-col">
          ...
          <div className="flex flex-col rounded-xl border border-border/20 bg-card/40 p-4">
            <div className="mb-3 flex items-center justify-between">
              <span className="text-xs font-semibold text-foreground">音频会话</span>
              <button
            ...
            {sessions.length === 0 ? (
              <p className="text-xs text-muted-foreground/70">
                {loadingSessions ? "正在扫描音频会话..." : "未检测到活动的音频会话"}
              </p>
            ) : (
                      ...
                      <button
                        onClick={() => {
                          if (matchingDevice) {
                            ...
                            showToast(`「对方」音源已设为 ${matchingDevice.name}`, "success");
                          } else {
                            showToast("未找到匹配的输出设备", "error");
                          }
                        }}
      ...
      <div className="flex items-start gap-3 rounded-xl border border-border/20 bg-card/40 px-4 py-3">
        ...
        <p className="text-xs text-muted-foreground/70">
          <span className="font-medium text-foreground">双用途音频：</span>{" "}
          在<span className="text-sky-400 font-medium">线上会议</span>中，「你」捕获你的麦克风，「对方」通过系统回环捕获远端音频。
          在<span className="text-purple-400 font-medium">线下会议</span>中，两个音源共用房间麦克风，并由说话人分离来区分不同声音。
        </p>
      </div>
    </div>
  );
...
function PartyPanel({
  ...
  const inputDevices = allDevices.filter((d) => d.group === "麦克风");
  const outputDevices = allDevices.filter((d) => d.group === "扬声器 / 输出");
  ...
  function handleProviderChange(newProvider: STTProviderType) {
    ...
    if (newProvider === "sherpa_onnx" || newProvider === "ort_streaming" || newProvider === "parakeet_tdt") {
      ...
      const DEFAULT_MODEL_PER_ENGINE: Record<string, string> = {
        sherpa_onnx: "streaming-zipformer-multi",
      ...
      <div className={`flex items-center gap-3 rounded-t-xl bg-gradient-to-r ${c.headerGrad} to-transparent border-b border-border/20 px-4 py-3`}>
        ...
        <div className="min-w-0 flex-1">
          ...
          {description ? (
            ...
            <p className="mt-0.5 text-meta text-muted-foreground/60">音频源与识别</p>
          )}
          {badge && (
        ...
        <div>
          ...
          >
            <option value="default">
              {loadingDevices ? "加载中..." : "默认"}
            </option>
            {inputDevices.length > 0 && (
              <optgroup label="麦克风">
                {inputDevices.map((d) => (
                  <option key={d.device.id} value={d.device.id}>
                    {d.device.name}{d.device.is_default ? "（默认）" : ""}
                  </option>
                ))}
              </optgroup>
            )}
            {outputDevices.length > 0 && (
              <optgroup label="扬声器 / 输出">
                {outputDevices.map((d) => (
                  <option key={d.device.id} value={d.device.id}>
                    {d.device.name}{d.device.is_default ? "（默认）" : ""}
                  </option>
        ...
        {(party.stt_provider === "web_speech" || party.stt_provider === "windows_native") &&
          ...
          <div className="flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/5 px-3 py-2">
            ...
            <p className="text-meta leading-relaxed text-amber-200/80">
              <span className="font-semibold text-amber-400">设备覆盖已启用。</span>{" "}
              {party.stt_provider === "web_speech" ? "Web Speech" : "Windows Speech"} 始终
              使用系统默认麦克风。会议期间，系统默认设备会被临时切换为所选设备，会议结束后自动恢复。
            </p>
          </div>
        )}
      </div>
...
function ProviderSelect({
  ...
  function handleStealConfirm() {
    ...
    if (!fallback) {
      showToast("没有可用的回退语音识别引擎。请先配置 API 密钥或下载本地模型。", "error");
      setStealTarget(null);
      return;
    }
    ...
    showToast(
      `${stealLabel} 已移至${thisRole === "you" ? "「你」" : "「对方」"}，${otherRole === "you" ? "「你」" : "「对方」"}回退为 ${fallbackLabel}。`,
      "info"
    );
                ...
                return (
                  ...
                  >
                    ...
                    <span className="flex-1 text-left">
                      {opt.label}
                      {locked && (
                        <span className="block text-meta text-muted-foreground/50">「{otherPartyLabel}」正在使用</span>
                      )}
                    </span>
          ...
          {stealTarget && (
            <div className="border-t border-border/20 bg-amber-500/5 px-3 py-2.5">
              <p className="text-meta leading-relaxed text-amber-200/80 mb-2">
                ...
                同一时间只能在一路音源上运行。要切换到这一方吗？
                另一方将回退到{" "}
                <span className="font-medium">
                  {(() => {
                    const fb = findExclusiveFallback();
                    return fb ? (STT_OPTIONS.find(o => o.value === fb)?.label ?? fb) : "无可用引擎";
                  })()}
                </span>。
              </p>

...
const PROMPT_FIELDS: {
  ...
  { key: "system_prompt", label: "系统提示词", description: "定义 AI 在会议中的角色与行为" },
  { key: "summary_prompt", label: "总结提示词", description: "会后总结的结构与重点" },
  { key: "question_detection_prompt", label: "问题检测", description: "从转写文本中检测并优先处理提问的规则" },
];
  ...
  return (
    <div className="rounded-xl border border-border/30 bg-card/60 p-4 shadow-lg">
      <p className="mb-3 text-xs font-medium text-foreground">
        {mode === "clone" ? "克隆场景为：" : "新场景名称："}
      </p>
      <input
        value={name}
        onChange={(e) => setName(e.target.value)}
        placeholder="例如：销售电话、1:1 会议"
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-border/30 bg-card/50 p-5">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <label className="text-sm font-medium text-foreground">当前场景</label>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {allScenarios.map((scenario) => (
            ...
            >
              {scenario.name}
              {scenario.is_custom && (
                <span className="ml-1.5 text-[10px] text-muted-foreground/50">自定义</span>
              )}
            </button>
          ))}
        </div>
        ...
        {cloneSourceId && (
          <div className="mt-3">
            <CreateScenarioDialog
              mode="clone"
              initialName={`${currentTemplate.name}（副本）`}
